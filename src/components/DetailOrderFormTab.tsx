...
export default function DashboardTab({
  ...
  const filteredOrders = React.useMemo(() => {
    return orders.filter((o) =>
      isDateInBounds(o.order_date || o.created_at || (o as any).date || (o as any).createdAt, startMs, endMs)
    );
  }, [orders, startMs, endMs]);

  const filteredPayments = React.useMemo(() => {
    return payments.filter((p) =>
      isDateInBounds(p.payment_date || p.created_at || (p as any).date || (p as any).timestamp, startMs, endMs)
    );
  ...
  const filteredCrmQuotations = React.useMemo(() => {
    return (crmQuotations || []).filter((q) =>
      isDateInBounds(
        q.created_at || q.validUntil || (q as any).date || (q as any).timestamp || (q as any).order_date || (q as any).createdAt,
        startMs,
        endMs
      )
    );
  ...
  const filteredCrmPayments = React.useMemo(() => {
    return (crmPayments || []).filter((cp) =>
      isDateInBounds(
        cp.payment_date || (cp as any).created_at || (cp as any).date || (cp as any).timestamp,
        startMs,
        endMs
      )
    );
  ...
  const { totalFurnitureBusiness, moneyReceived, moneyDue, finalizedOrdersCount } = React.useMemo(() => {
    ...
    Object.entries(invoiceGroups).forEach(([invoiceKey, groupOrders]: [string, Order[]]) => {
      ...
      let invoiceTotal = 0;
      const explicitTotalFromPayment = matchingPayments.find(
        (p) => typeof p.total_amount === 'number' && p.total_amount > 0
      )?.total_amount;
      const explicitTotalFromCrm = matchingCrmPayments.find(
        (cp) => typeof cp.total_amount === 'number' && cp.total_amount > 0
      )?.total_amount;
      ...
      if (explicitTotalFromPayment) {
        ...
        invoiceTotal = groupOrders.reduce((sum, ord) => {
          ...
          // Check if this order links to a CRM Quotation with totalAmount
          const linkedQuote = (crmQuotations || []).find((q) => {
            const qId = String(q.id || '').trim().toLowerCase();
            return qId && (ord.id === q.id || (ord.special_notes && ord.special_notes.toLowerCase().includes(qId)));
          });
          if (linkedQuote && Number(linkedQuote.totalAmount) > 0) {
            return sum + Number(linkedQuote.totalAmount);
          }
      ...
      const directOrderAdvanceSum = groupOrders.reduce((acc, o) => {
        const adv =
          Number(o.advance_paid) ||
          Number((o as any).advance) ||
          Number((o as any).received_amount) ||
          Number((o as any).advancePaid) ||
          0;
    ...
    // Standalone CRM Quotations (Prevent duplicate counting when converted to orders)
    (filteredCrmQuotations || []).forEach((q) => {
      if (!q) return;
      const qIdNorm = String(q.id || '').trim().toLowerCase();

      // Check if this quotation was already converted to an order
      const isMatchedToOrder = filteredOrders.some((o) => {
        const oIdNorm = String(o.id || '').trim().toLowerCase();
        const parentIdNorm = String(o.parent_order_id || '').trim().toLowerCase();
        const notes = String(o.special_notes || '').toLowerCase();
        return (
          (qIdNorm && oIdNorm === qIdNorm) ||
          (qIdNorm && parentIdNorm === qIdNorm) ||
          (qIdNorm && notes.includes(qIdNorm))
        );
      });
      ...
      if (!isMatchedToOrder) {
        ...
        const qReceived =
          Number(q.received_amount) ||
          Number((q as any).advance_paid) ||
          Number((q as any).receivedAmount) ||
          0;

        if (isApproved && qTotal > 0) {
  ...
  }, [invoiceGroups, filteredOrders, filteredPayments, filteredCrmPayments, filteredCrmQuotations, crmQuotations]);
  ...
  const quotationStats = React.useMemo(() => {
    ...
    quotes.forEach((item) => {
      ...
      if (
        ...
        // DRAFT, QUOTATION PENDING, PENDING, NEW INQUIRY, etc.
        draft++;
      }
    });
